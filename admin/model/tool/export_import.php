...
class ModelToolExportImport extends Model {
	...
	protected function storeProductIntoDatabase( &$product, &$languages, &$product_fields, $exist_table_product_tag, $exist_meta_title, &$layout_ids, &$available_store_ids, &$manufacturers, &$weight_class_ids, &$length_class_ids, &$url_alias_ids ) {
		...
		$fias_id = (int)$product['fias_id'];
		$link = $this->db->escape($product['link']);
		...
		$sql .= "`tax_class_id`,`viewed`,`length`,`width`,`height`,`length_class_id`,`sort_order`,`subtract`,`minimum`, fias_id, outer_link) VALUES ";
		...
		$sql .= "$tax_class_id,$viewed,$length,$width,$height,'$length_class_id','$sort_order','$subtract','$minimum','$fias_id','$link');";
		$this->db->query($sql);
		foreach ($languages as $language) {
			$language_code = $language['code'];
			$language_id = $language['language_id'];
	...
	protected function uploadProducts( &$reader, $incremental, &$available_product_ids=array() ) {
		...
		for ($i=0; $i<$k; $i+=1) {
			...
			$fias_id = $this->getCell($data,$i,$j++,'1');
			$city = $this->getCell($data,$i,$j++,'1');
			$link = $this->getCell($data,$i,$j++,'1');
			$product = array();
	...
	protected function validateAttributeColumns( &$reader ) {
		...
		foreach ($query->rows as $row) {
			if ($export_import_settings_use_attribute_group_id) {
				...
				if (!isset($attribute_groups[trim($attribute_group_name)])) {
					$attribute_groups[trim($attribute_group_name)] = array();
				}
				if ($export_import_settings_use_attribute_id) {
					$attribute_id = $row['attribute_id'];
					if (!is_null($attribute_id)) {
						$attribute_groups[trim($attribute_group_name)][$attribute_id] = true;
					}
				} else {
					$attribute_name = htmlspecialchars_decode($row['attribute_name']);
					if (!is_null($attribute_name)) {
						$attribute_groups[trim($attribute_group_name)][trim($attribute_name)] = true;
					}
				}
			}
		}
		...
		for ($i=1; $i<$k; $i+=1) {
			...
			if ($export_import_settings_use_attribute_group_id) {
				...
				if ($export_import_settings_use_attribute_id) {
					...
					if (!isset($attribute_groups[trim($attribute_group_name)][$attribute_id])) {
					...
					if (!isset($attribute_groups[trim($attribute_group_name)][trim($attribute_name)])) {
...
//		}
	...
	protected function getProducts( &$languages, $default_language_id, $product_fields, $exist_meta_title, $offset=null, $rows=null, $min_id=null, $max_id=null ) {
		...
		$sql .= "  p.minimum, ";
		$sql .= "  p.fias_id, ";
		$sql .= "  p.outer_link as link, ";
		$sql .= "  CONCAT(f.shortname,' ',f.offname) as city, ";
		...
		$sql .= "LEFT JOIN `fias` f ON f.fias_id=p.fias_id ";
		if (isset($min_id) && isset($max_id)) {
			$sql .= "WHERE p.product_id BETWEEN $min_id AND $max_id ";
		}
		$sql .= "GROUP BY p.product_id ";
	...
	function populateProductsWorksheet( &$worksheet, &$languages, $default_language_id, &$price_format, &$box_format, &$weight_format, &$text_format, $offset=null, $rows=null, &$min_id=null, &$max_id=null) {
		...
		$worksheet->getColumnDimensionByColumn($j++)->setWidth(0);
		$worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('city'),8)+1);
		$worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('link'),8)+1);
	...
	protected function getAddresses( $min_id, $max_id ) {
		...
		$sql .= "INNER JOIN `".DB_PREFIX."zone` z ON z.country_id=a.country_id AND z.zone_id=a.zone_id ";
	...
	public function download( $export_type, $offset=null, $rows=null, $min_id=null, $max_id=null) {
		...
		try {
			...
			$objWriter->setPreCalculateFormulas(false);
                        $file  =  'php://output';                        
                        $file =  tempnam (DIR_DOWNLOAD , "export-xlsx.");                        
                        $objWriter->save($file);
                        readfile($file);
	...
	public function getNotifications() {
		...
		$result = $this->curl_get_contents("http://www.mhccorp.com/index.php?route=information/message&type=tool_export_import_3_2&language_code=$language_code");
